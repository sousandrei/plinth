...
use crate::sync::{discovery, identity, server, tls, PairingState, PeerRegistry};
...
/// the listener, then start mDNS discovery. Returns the runtime handle
/// for the caller to register with Tauri.
///
...
pub async fn start(handle: AppHandle, db: SqlitePool) -> Result<SyncRuntime, AppError> {
    ...
    discovery::spawn(handle, db, peers.clone(), server_handle.local_addr.port());
